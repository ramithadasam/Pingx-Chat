...
export default function RemoveFriendPage() {
  ...
  const confirmSingleRemove = (id: string) => {
  ...
  return (
    ...
    >
      <PageHeader 
        ...
        rightElement={<UserMinus className="w-6 h-6 text-[#FFC6DA]" />}
      />
            ...
            <div key={contact.id} className="flex items-center gap-3 p-3 bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.05)] rounded-[20px]">
              <div 
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 cursor-pointer ${
                  selected.has(contact.id) ? 'bg-[#EF4444] border-[#EF4444]' : 'border-[rgba(255,255,255,0.3)]'
                }`}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[rgba(255,255,255,0.5)] text-sm truncate">{contact.lastMessage}</p>
              </div>
              
              <button 
                onClick={() => confirmSingleRemove(contact.id)}
      ...
      <ConfirmModal 
        ...
        description={
          contactToRemove 
            ? `Are you sure you want to remove ${getContactName(contactToRemove)} from your friends?`
            : `Are you sure you want to remove ${selected.size} friends?`
        }
        onConfirm={handleRemove}
        onCancel={() => { setModalOpen(false); setContactToRemove(null); }}
      />
    </motion.div>
  );
}
